...
class DistributedWeightedSampler(DistributedSampler):
    ...
    def __init__(
        ...
        if not (hasattr(dataset, "sample_weights")):
            raise AssertionError("Dataset must have sample_weights property for using DistributedWeightedSampler")
        super().__init__(
            dataset,
    ...
    def __iter__(self) -> Iterator:
        ...
        if not (len(indices) == self.total_size):
            raise AssertionError("assertion failed: len(indices) == self.total_size")

        # subsample
        indices = indices[self.rank : self.total_size : self.num_replicas]
        if not (len(indices) == self.num_samples):
            raise AssertionError("assertion failed: len(indices) == self.num_samples")
...
class MemoryEfficientDistributedWeightedSampler(DistributedSampler):
    ...
    def __init__(
        ...
        if not (hasattr(dataset, "dataset_weights")):
            raise AssertionError(
...
class MemoryEfficientDistributedWeightedSamplerLessRepeat(DistributedSampler):
    ...
    The reduced-repeat behavior comes from permuting only the rank-local subset of dataset indices.
    """
    ...
    def __init__(
        ...
        if not (hasattr(dataset, "dataset_weights")):
            raise AssertionError(
                "Dataset must have dataset_weights property for using "
    ...
    def _new_sampler(self, sample_size: int) -> RandomSampler:
        if not (self.shuffle):
            raise AssertionError("assertion failed: self.shuffle")
    ...
    def _in_rank_next_index_for_dataset(self, dataset_idx: int) -> int:
        ...
        if next_sampler_idx is None:
            ...
            next_sampler_idx = safe_next(self.individual_dataset_sampler[dataset_idx])
            if not (next_sampler_idx is not None):
                raise AssertionError("assertion failed: next_sampler_idx is not None")
