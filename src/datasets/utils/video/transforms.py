...
def uniform_crop(images, size, spatial_idx, boxes=None, scale_size=None):
    ...
    Returns:
        ...
        cropped_boxes (ndarray or None): the cropped boxes with dimension of
            `num boxes` x 4.
    """
    if not (spatial_idx in [0, 1, 2]):
        raise AssertionError("assertion failed: spatial_idx in [0, 1, 2]")
...
def color_normalization(images, mean, stddev):
    ...
    """
    if len(images.shape) == 3:
        if not (len(mean) == images.shape[0]):
            raise AssertionError("channel mean not computed properly")
        if not (len(stddev) == images.shape[0]):
            raise AssertionError("channel stddev not computed properly")
    elif len(images.shape) == 4:
        if not (len(mean) == images.shape[1]):
            raise AssertionError("channel mean not computed properly")
        if not (len(stddev) == images.shape[1]):
            raise AssertionError("channel stddev not computed properly")
    else:
        raise NotImplementedError(f"Unsupported dimension {len(images.shape)}")
...
def create_random_augment(
    ...
    if auto_augment:
        if not (isinstance(auto_augment, str)):
            raise AssertionError("assertion failed: isinstance(auto_augment, str)")
...
def random_sized_crop_img(
    ...
    if not (len(im.shape) == 3):
        raise AssertionError("Currently only support image for random_sized_crop")
    h, w = im.shape[1:3]
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray) or isinstance(clip[0], torch.Tensor):
            if clip[0].shape[-1] == 3:
                ...
                if not (clip[0].shape[0] == 3):
                    raise AssertionError("assertion failed: clip[0].shape[0] == 3")
