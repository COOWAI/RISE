...
class ClipToTensor(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            h, w, ch = clip[0].shape
            if not (ch == self.channel_nb):
                raise AssertionError("Got {0} instead of 3 channels".format(ch))
            ...
            actual_type = type(clip[0])
            raise TypeError(
                f"Expected numpy.ndarray or PIL.Image or torch.Tensor            but got list of {actual_type}"
            )
        ...
        for img_idx, img in enumerate(clip):
            if isinstance(img, np.ndarray):
                ...
                actual_type = type(clip[0])
                raise TypeError(f"Expected numpy.ndarray or PIL.Image                but got list of {actual_type}")
            img = convert_img(img)
            np_clip[:, img_idx, :, :] = img
