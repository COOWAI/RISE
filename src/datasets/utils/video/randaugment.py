...
def _select_rand_weights(weight_idx=0, transforms=None):
    ...
    if not (weight_idx == 0 or weight_idx == 1):
        raise AssertionError("assertion failed: weight_idx == 0 or weight_idx == 1")
    if weight_idx == 0:
...
def rand_augment_transform(config_str, hparams):
    ...
    if not (config[0] == "rand"):
        raise AssertionError('assertion failed: config[0] == "rand"')
    config = config[1:]
    for c in config:
        cs = re.split(r"(\d.*)", c)
        if len(cs) < 2:
        ...
        if key == "mstd":
            ...
            if not (NotImplementedError):
                raise AssertionError("assertion failed: NotImplementedError")
