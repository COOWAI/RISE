...
class TestViTGiant(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if free_bytes < 20 * 1024**3:  # 2x ViT-Giant + 10 iters OOM a shared/busy GPU
            self.skipTest(f"ViT-Giant test needs >=20GB free GPU mem; only {free_bytes / 1024**3:.1f}GB free")
