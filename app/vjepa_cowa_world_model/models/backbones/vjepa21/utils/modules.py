...
def rotate_queries_or_keys(x, pos, n_registers, has_cls_first):
    ...
    if not (D % 2 == 0):
        raise AssertionError("Embedding dimension must be a multiple of 2 for block matrix rotation")
...
class RoPEAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_sdpa:
            ...
            if self.is_causal:  # match the SDPA is_causal path; the manual path previously leaked future tokens
                causal = torch.ones(attn.shape[-2], attn.shape[-1], dtype=torch.bool, device=attn.device).triu(1)
...
class Lambda_LinearWarmupHold:
    ...
    def __init__(self, lambda_value: float, start_iter: int = 15_000, end_iter: int = 30_000):
        if not (end_iter > start_iter):
            raise AssertionError("end_iter must be > start_iter")
