        ...
        if masks is None:
            ...
            for x_fpc in x:
                if gram_mode:
                    ...
                    x_2d = x_fpc.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)  # (B*T, C, H, W)
                    x_up = F.interpolate(x_2d, scale_factor=2, mode="bicubic", align_corners=False)
                    _, _, H_up, W_up = x_up.shape
                    x_up = x_up.view(B, T, C, H_up, W_up).permute(0, 2, 1, 3, 4)  # (B,C,T,H_up,W_up)

                    # Then we make the pass through the backbone
                    out = self.backbone(x_up)
                    B, N, D = out.shape
                    H_up_patches = W_up_patches = int(H_up // 16)  # We have hardcoded this to the patch size
                    if T == 1:
                        T_up_patches = 1  # In this case, it is a LVD image
                    else:
                        T_up_patches = int(T // 2)  # We have hardcoded this to the tubelet size
                    out_3d = out.view(B, T_up_patches, H_up_patches, W_up_patches, D)  # (bs, T, H, W, D)
