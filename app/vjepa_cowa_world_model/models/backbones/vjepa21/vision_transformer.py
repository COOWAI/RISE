...
from app.vjepa_cowa_world_model.models.backbones.vjepa21.utils.modules import Block
from app.vjepa_cowa_world_model.models.backbones.vjepa21.utils.patch_embed import PatchEmbed, PatchEmbed3D
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None, training=False):
        ...
        if self.check_temporal_dim(x.shape):
            if not (self.patch_embed_img is not None):
                raise AssertionError("assertion failed: self.patch_embed_img is not None")
    ...
    def interpolate_pos_encoding(self, x, pos_embed):
        ...
        if self.is_video:
            ...
            if not (N_h * N_w * N_t == N):
                raise AssertionError("Positional embedding initialized incorrectly")
