...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        ...
        The forward SDE ensures that the condition distribution q_{t|0}(x_t | x_0) =
        N ( alpha_t * x_0, sigma_t^2 * I ). We further define lambda_t = log(alpha_t) - log(sigma_t),
        which is the half-logSNR (described in the DPM-Solver paper). Therefore, we implement the functions
        for computing alpha_t, sigma_t and lambda_t. For t in [0, T], we have:
        ...
        We support both discrete-time DPMs (trained on n = 0, 1, ..., N-1) and continuous-time DPMs
        (trained on t in [t_0, T]).

        1. For discrete-time DPMs:

            For discrete-time DPMs trained on n = 0, 1, ..., N-1, we convert the discrete steps
            to continuous time steps by:
                t_i = (i + 1) / N
            ...
            **Important**:  Please pay special attention for the args for `alphas_cumprod`:
                ...
                Specifically, DDPMs assume that
                    q_{t_n | 0}(x_{t_n} | x_0) =
                    N ( \sqrt{\hat{alpha_n}} * x_0, (1 - \hat{alpha_n}) * I ).
                Therefore, the notation \hat{alpha_n} is different from the notation alpha_t
                in DPM-Solver. In fact, we have
        ...
        # For discrete-time DPMs, given alphas_cumprod
        # (the \hat{alpha_n} array for n = 0, 1, ..., N - 1):
        ...
        if schedule == "discrete":
            if betas is not None:
                ...
                if not (alphas_cumprod is not None):
                    raise AssertionError("assertion failed: alphas_cumprod is not None")
...
def model_wrapper(
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if not (classifier_fn is not None):
                raise AssertionError("assertion failed: classifier_fn is not None")
            t_input = get_model_input_time(t_continuous)
    ...
    if not (model_type in ["noise", "x_start", "v", "score"]):
        raise AssertionError('assertion failed: model_type in ["noise", "x_start", "v", "score"]')
    if not (guidance_type in ["uncond", "classifier", "classifier-free"]):
        raise AssertionError('assertion failed: guidance_type in ["uncond", "classifier", "classifier-free"]')
    return model_fn
...
class DPM_Solver:
    def __init__(
        ...
        [1] Chitwan Saharia, William Chan, Saurabh Saxena, Lala Li, Jay Whang, Emily Denton,
            ...
            language understanding. arXiv preprint arXiv:2205.11487, 2022b.
        """
        self.model = lambda x, t: model_fn(x, t.expand((x.shape[0])))
        self.noise_schedule = noise_schedule
        if not (algorithm_type in ["dpmsolver", "dpmsolver++"]):
            raise AssertionError('assertion failed: algorithm_type in ["dpmsolver", "dpmsolver++"]')
    ...
    def denoise_to_zero_fn(self, x, s):
        """
        Denoise at the final step, which is equivalent to solve the ODE from lambda_s to infty
    ...
    def singlestep_dpm_solver_third_update(
        ...
        Args:
            ...
            model_s: A pytorch tensor. The model function evaluated at time `s`.
                If `model_s` is None, we evaluate the model by `x` and `s`; otherwise we directly use it.
            model_s1: A pytorch tensor. The model function evaluated at time `s1`
    ...
    def dpm_solver_adaptive(
        ...
        Args:
            ...
            atol: A `float`. The absolute tolerance of the solver. For image data,
                the default setting is 0.0078, followed [1].
            rtol: A `float`. The relative tolerance of the solver. The default setting is 0.05.
            theta: A `float`. The safety hyperparameter for adapting the step size.
                The default setting is 0.9, followed [1].
            t_err: A `float`. The tolerance for the time. We solve the diffusion ODE until
                the absolute error between the current time and `t_0` is less than `t_err`.
        ...
        if order == 2:
            ...
            def _lower_update_order_2(x, s, t):
                return self.dpm_solver_first_update(x, s, t, return_intermediate=True)

            def _higher_update_order_2(x, s, t, **kwargs):
                return self.singlestep_dpm_solver_second_update(
                    x,
                    s,
                    t,
            ...
            lower_update = _lower_update_order_2
            higher_update = _higher_update_order_2
        elif order == 3:
            r1, r2 = 1.0 / 3.0, 2.0 / 3.0

            def _lower_update_order_3(x, s, t):
                return self.singlestep_dpm_solver_second_update(
                    x,
                    s,
                    t,
            ...
            def _higher_update_order_3(x, s, t, **kwargs):
                return self.singlestep_dpm_solver_third_update(
                    x,
                    s,
                    t,
            ...
            lower_update = _lower_update_order_3
            higher_update = _higher_update_order_3
        else:
    ...
    def sample(
        ...
        We support the following algorithms for both noise prediction model and data prediction model:
            - 'singlestep':
                Singlestep DPM-Solver (i.e. "DPM-Solver-fast" in the paper), which combines
                different orders of singlestep DPM-Solver. We combine all the singlestep solvers
                with order <= `order` to use up all the function evaluations (steps).
                ...
                Given a fixed NFE == `steps`, the sampling procedure is:
                    ...
                    - If `order` == 3:
                        ...
                        - If steps % 3 == 0, we use (K - 2) steps of singlestep DPM-Solver-3,
                          1 step of singlestep DPM-Solver-2 and 1 step of DPM-Solver-1.
            ...
            - 'multistep':
                ...
                Given a fixed NFE == `steps`, the sampling procedure is:
                    ...
                    - If `order` == 3:
                        - We firstly use 1 step of DPM-Solver-1, then 1 step of multistep
                          DPM-Solver-2, then (K - 2) step of multistep DPM-Solver-3.
            - 'singlestep_fixed':
                Fixed order singlestep DPM-Solver (i.e. DPM-Solver-1,
                singlestep DPM-Solver-2 or singlestep DPM-Solver-3). We use singlestep
                DPM-Solver-`order` for `order`=1 or 2 or 3, with total
                [`steps` // `order`] * `order` NFE.
            - 'adaptive':
                ...
                to balance the computatation costs (NFE) and the sample quality.
                    ...
                    - If `order` == 3, we use DPM-Solver-23 which combines
                      singlestep DPM-Solver-2 and singlestep DPM-Solver-3.
        ...
        if return_intermediate:
            if not (
                ...
                raise AssertionError("Cannot use adaptive solver when saving intermediate values")
        if self.correcting_xt_fn is not None:
            if not (
                ...
                raise AssertionError("Cannot use adaptive solver when correcting_xt_fn is not None")
        device = x.device
        intermediates = []
        with torch.no_grad():
            if method == "adaptive":
                ...
                if not (steps >= order):
                    raise AssertionError("assertion failed: steps >= order")
                timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=steps, device=device)
                if not (timesteps.shape[0] - 1 == steps):
                    raise AssertionError("assertion failed: timesteps.shape[0] - 1 == steps")
